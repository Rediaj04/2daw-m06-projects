...
const productos: { nombre: string; plataforma?: string }[] = [];

// Sobrecarga para agregar productos
function añadirProducto(nombre: string): void;
function añadirProducto(nombre: string, plataforma: string): void;
function añadirProducto(nombre: string, plataforma?: string): void {
    if (plataforma) {
...
const esEmailValido = (email: string): boolean => email.includes("@");
...
function mostrar(tipo: string) {
    const tabla = document.getElementById("tablaProductos") as HTMLTableElement;
    if (tabla) {
